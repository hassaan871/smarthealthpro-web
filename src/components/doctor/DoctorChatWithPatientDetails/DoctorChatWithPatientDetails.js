...
function DoctorChatWithPatientDetails() {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      sender: "patient",
      text: "Hello, Doctor. I have been experiencing some headaches recently.",
    },
    {
      id: 2,
      sender: "doctor",
      text: "Hello! How long have you been having these headaches?",
    },
  ]);
  ...
  const chatSummaries = [
    {
      date: "2024-09-01",
      summary:
        "Discussed management strategies for fluctuating blood glucose levels in diabetes.",
    },
    {
      date: "2024-08-15",
      summary:
        "Reviewed lab results indicating elevated LDL cholesterol, prescribed statins to reduce cardiovascular risk.",
    },
  ...
  const handleUpdatePriority = async () => {
    ...
    if (!appointment || !appointment._id || !newPriority) {
      setUpdateStatus({
        type: "error",
        message: "Invalid appointment data or priority",
      });
      return;
    }
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/appointment/updateAppointment/${appointment._id}`,
        {
          method: "PUT",
          headers: {
      ...
      <Row className="h-100">
          ...
          <Card className="h-100">
            ...
            <Card.Body className="overflow-auto">
              ...
              {updateStatus && (
                <Alert
                  variant={
                    updateStatus.type === "success" ? "success" : "danger"
                  }
                  className="mt-3"
                >
        ...
        <Col md={3} className="h-100">
          <Card className="h-100">
            ...
            >
              ...
              <Nav.Item>
                <Nav.Link eventKey="prescriptionNotes">
        ...
        <Col md={6} className="h-100">
          <Card className="h-100">
            ...
            >
              <span>
                {chatMode === "chat" ? "Chat with Patient" : "Add Notes"}
              </span>
              <Form.Check
                type="switch"
                id="chat-mode-switch"
                label={
                  chatMode === "chat" ? "Switch to Notes" : "Switch to Chat"
                }
                onChange={() =>
                  setChatMode(chatMode === "chat" ? "notes" : "chat")
                }
              />
            </Card.Header>
